...
- `POST /explain-narrative` — same as `/explain`, plus a plain-English retention narrative generated by a local LLM via Ollama (see below)
- `POST /analyze-batch` — accepts a list of customers (1-500), scores all of them, aggregates the SHAP drivers shared across the at-risk group, and returns a group-level retention narrative (see below)
...
SHAP values are precise but not something a non-technical account manager wants to read. `POST /explain-narrative` takes the same SHAP contributions from `/explain` and feeds them to a local Ollama model (`narrate.py`) to generate a short, plain-English retention writeup with a concrete recommended action:

```bash
ollama serve   # if not already running
curl -X POST http://127.0.0.1:8000/explain-narrative -H "Content-Type: application/json" -d '{
  "gender": "Female", "SeniorCitizen": 0, "Partner": "Yes", "Dependents": "No",
  "tenure": 1, "PhoneService": "No", "MultipleLines": "No phone service",
  "InternetService": "DSL", "OnlineSecurity": "No", "OnlineBackup": "Yes",
  "DeviceProtection": "No", "TechSupport": "No", "StreamingTV": "No",
...
Everything runs on-device — no data or SHAP values leave the machine. The model is configurable via `OLLAMA_MODEL` (defaults to `llama3.2:1b`) and `OLLAMA_HOST` env vars, so swapping the LLM backing this endpoint is a one-line change. Small local models occasionally misreason about *why* a factor matters even when they correctly identify its direction (e.g. calling a 1-month tenure "extended") — a known limitation of 1-3B instruct models, not something to over-engineer around for a local-dev demo.

### Batch risk analysis across a group of customers

A retention team doesn't review customers one at a time — they want to know which segment of their book is at risk and why. `POST /analyze-batch` takes a list of customers, scores all of them, and instead of feeding raw per-customer SHAP values to the LLM (which small local models reliably mangle across many rows), it first aggregates them in `explain.py` (`aggregate_churn_drivers`) into the churn drivers shared by the most at-risk customers — then narrates *that* summary:

```bash
curl -X POST http://127.0.0.1:8000/analyze-batch -H "Content-Type: application/json" -d '{
...
```json
{
  ...
  "total_customers": 15,
  "high_risk_count": 4,
  "average_churn_probability": 0.346,
  "top_shared_drivers": [
    {"feature": "cat__Contract_Month-to-month", "customers_affected": 10, "avg_shap_value": 0.54},
    {"feature": "num__tenure", "customers_affected": 7, "avg_shap_value": 0.51}
  ],
  "customers": [{"index": 0, "churn_prediction": 1, "churn_probability": 0.71}, "..."],
  "narrative": "Based on the analysis, I would rate the overall risk level of this batch as moderate to high..."
}
```

This is the same pattern as `/explain-narrative` (aggregate first, then summarize) applied at the group level instead of the single-customer level — pre-aggregating with pandas/SHAP before the LLM call keeps the prompt small and the numbers exact, and only hands the LLM the part it's actually good at: turning a structured summary into prose.
